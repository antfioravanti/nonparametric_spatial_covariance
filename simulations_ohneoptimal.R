...
grid$distance <- sqrt(grid$x^2 + grid$y^2)
grid1 <- grid[order(grid$distance), ]
grid <- data.frame(x = grid1$x, y = grid1$y)
...
plot_matrix_image(true_covariance, main = paste0("Wahre Kovarianzmatrix\n",
                                              "phi1=phi2=", phi,", ",
                                              "sigma=", sigma))
plot_matrix(grid, sigma, phi)
#-------------------------------------------------------------------------------
...
ggplot(grid, aes(x = x, y = y, color = sim, shape = type)) +
  ...
  labs(title = "Simuliertes räumliches gaußsches Zufallsfeld",
       x = "x-Koordinate",
       y = "y-Koordinate",
       color = "Wert",
       shape = "Point Type") +
...
bandwidths = c(0.01, 0.05, 0.1) # bandwidths
